// 목표 날짜 설정 (2024년 7월 19일)
const targetDate = new Date("2024-07-19T00:00:00Z");
...
doors.forEach((door, index) => {
  door.addEventListener('click', () => {
    ...
    // 각 날짜에 해당하는 날짜를 계산
    const openDate = new Date(2024, 5, 26 + index); // 2024년 6월 26일부터 시작
    ...
    if (now.getTime() > openDate.getTime()) {
      ...
      //체크박스가 체크되면서 카드 뒤집어짐 방지
      const doorDiv = document.querySelector(`.day-${index + 1}`)
      const chk = doorDiv.querySelector('input')
